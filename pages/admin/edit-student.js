...
export default function EditStudent() {
    ...
    const checkAuth = async () => {
      try {
        ...
        try {
          ...
          // If we have a student ID, fetch the student data
          if (id) {
            try {
              const studentResponse = await axios.get(`/api/admin/student?id=${id}`);
              const studentData = studentResponse.data;
              
              if (!studentData) {
                setError('Student not found. Please try again or contact support.');
                setLoading(false);
                return;
              }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response;
      try {
        response = await axios.put('/api/admin/update-student', {
          id,
          ...formData,
          phone: formattedPhone,
          schoolId: school.id
  ...
  if (loading) {
    return (
      <Layout title="Edit Student | Bandhan">
